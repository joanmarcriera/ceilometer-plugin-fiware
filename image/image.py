        ...
        for endpoint in resources:
            for image in self._iter_images(manager.keystone, cache, endpoint):
                ...
                yield sample.Sample(
                    ...
                    type=sample.TYPE_GAUGE,
                    unit='#',
                    volume=(metadata["name"] if ("name" in metadata and metadata["name"]) else None),
                    user_id=None,
                    project_id=image.owner,
                    resource_id=image.id,
                    timestamp=timeutils.isotime(),
                ...
                yield sample.Sample(
                    ...
                    type=sample.TYPE_GAUGE,
                    unit='#',
                    volume=(metadata["status"] if ("status" in metadata and metadata["status"]) else None),
                    user_id=None,
                    project_id=image.owner,
                    resource_id=image.id,
                    timestamp=timeutils.isotime(),
                ...
                if "properties" in metadata and metadata["properties"] and "nid" in  metadata["properties"] and metadata["properties"]["nid"]:
                    yield sample.Sample(
                        name='image.nid',
                        type=sample.TYPE_GAUGE,
                        unit='#',
                        volume=metadata["properties"]["nid"],
                        user_id=None,
                        project_id=image.owner,
                        resource_id=image.id,
                        timestamp=timeutils.isotime(),
